...
- [x] 编写 FastAPI 静态目录挂载代码（`app.mount("/static", ...)`）

### 第二阶段：核心 API 开发

- [x] 开发 `wish` 路由，实现概率抽取逻辑
- [x] 开发 `recipes` CRUD 接口，支持 JSONB 格式的食材和步骤存取

### 第三阶段：前端集成与适配

- [x] 将原始 HTML 中的模拟数据 `RECIPE_POOL` 替换为异步 Fetch 请求
- [x] 开发 `admin.html` 页面，实现动态添加食材行和步骤行
- [x] 为 Admin 页面补充食谱图片上传与预览能力
- [x] 使用 Tailwind 的 `sm:` / `md:` / `lg:` 前缀完成全设备适配

---

## 8. 优化计划 (Optimization Plan)

### A. 功能优化

- [ ] 去除状态标识（"准备好了"状态药丸），简化界面冗余元素
- [ ] 支持自定义灵感数量，不局限于固定的 1 次 / 10 次选项
- [ ] 去除固定稀有度百分比展示，改为在抽奖配置时设置（可选配置）
- [ ] 将提示标语放置在抽奖选择配置的下方；在菜品类别旁放置稀有度选择下拉框
- [ ] 在未抽取时，可将每个类别的菜推荐一个作为默认展示（可选）
- [ ] 修改菜谱字段：除基础信息（名称/分类/图片）外，只保留 **食材** 和 **关键描述词**，移除 `steps` 步骤字段
- [ ] 抽奖配置中支持根据关键字筛选（关键字来源于食材和特点/描述词）
- [ ] 菜谱支持通过 JSON 数据快速批量导入
- [ ] 支持多次选择，并可记录每次确认的选择结果（历史记录）

### B. UI 优化

- [ ] 简化文字信息，减少冗余说明
- [ ] 不局限于"晚饭"主题，改为更通用的餐饮灵感表述
- [ ] 抽奖结果卡片显示范围加大
- [ ] 抽奖结果排版优化
- [ ] 添加抽奖音效
- [ ] 针对移动端进行深度优化
- [ ] 进入页面直接进行抽奖配置（跳过引导，直达操作）
- [ ] 抽奖按钮点击后，结果显示为一个独立的全屏页面
