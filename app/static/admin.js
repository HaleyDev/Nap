...
const batchJsonInput = document.getElementById('batch-json');
...
function releasePreviewUrl() {
    if (state.previewObjectUrl) { URL.revokeObjectURL(state.previewObjectUrl); state.previewObjectUrl = ''; }
}

function setImagePreview(src) {
    const value = String(src || '').trim();
    releasePreviewUrl();
    if (!value) { imagePreview.classList.add('empty'); imagePreview.textContent = '未选择图片'; return; }
    setPreviewContent(`
    ...
    img?.addEventListener('error', () => {
        setPreviewContent(`<div class="image-preview-fallback">图片无法显示</div>`);
    }, { once: true });
}
...
function ensureInitialRows() {
    ...
    if (!keywordsList.children.length) {
        addDynamicRow(keywordsList, '例如：家常、辣、快手');
    }
}

function collectRows(target) {
    return Array.from(target.querySelectorAll('input')).map(i => i.value.trim()).filter(Boolean);
}
...
function replaceRows(target, values, placeholder) {
    ...
    if (!target.children.length) addDynamicRow(target, placeholder);
}
...
function fillForm(recipe) {
    ...
    replaceRows(keywordsList, recipe.keywords || [], '例如：家常');
...
function validateRecipePayload(payload) {
    if (!payload.name) return '请填写菜名';
    if (!payload.img_url) return '请先上传图片，或手动填写图片路径';
    if (!payload.ingredients.length) return '至少填写一个食材';
    return '';
}
    ...
    return `
        <article class="recipe-item">
            ...
            <div class="space-y-3 p-4">
                ...
                ${(recipe.keywords || []).length ? `<div class="flex flex-wrap gap-1">${recipe.keywords.map(k => `<span class="pill-tag text-xs">${escapeHtml(k)}</span>`).join('')}</div>` : ''}
                <div class="flex flex-wrap gap-2">
                    ${recipe.ingredients.map(item => `<span class="pill-tag">${escapeHtml(item)}</span>`).join('')}
                </div>
                <div class="flex gap-2 pt-1">
                    <button type="button" class="mini-action" data-edit-recipe-id="${recipe.id}">编辑</button>
                    <button type="button" class="mini-action mini-action-danger" data-delete-recipe-id="${recipe.id}">删除</button>
                </div>
            </div>
        </article>
    `;
...
async function loadRecipes() {
    ...
    recipeList.innerHTML = '<div class="empty-state md:col-span-2">正在同步…</div>';
    const response = await fetch('/api/v1/recipes', { headers: { Accept: 'application/json' } });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.detail || '读取菜谱失败');
    state.recipes = payload;
    recipeCount.textContent = `${payload.length} 道菜`;
    if (!payload.length) { recipeList.innerHTML = '<div class="empty-state md:col-span-2">当前没有菜谱</div>'; return; }
...
async function submitRecipe(event) {
    ...
    const payload = {
        ...
        keywords: collectRows(keywordsList),
    ...
    try {
        ...
        if (!response.ok) throw new Error(getErrorMessage(result.detail, '提交失败'));
        resetRecipeForm();
...
async function uploadRecipeImage(options = {}) {
    ...
    if (!file) { if (!silent) setFormStatus('请先选择图片文件', 'error'); throw new Error('请先选择图片文件'); }
    if (!silent) setFormStatus('上传图片中', 'busy');
    const payload = new FormData();
    payload.append('file', file);
    const response = await fetch('/api/v1/uploads/recipe-image', { method: 'POST', body: payload });
    const result = await response.json();
    if (!response.ok) throw new Error(getErrorMessage(result.detail, '图片上传失败'));
    imageUrlInput.value = result.img_url;
    setImagePreview(result.img_url);
    if (!silent) setFormStatus(`图片已上传：${result.original_filename}`, 'success');
    return result.img_url;
}

async function deleteRecipe(recipeId) {
    setFormStatus(`删除中 #${recipeId}`, 'busy');
    const response = await fetch(`/api/v1/recipes/${recipeId}`, { method: 'DELETE' });
    if (!response.ok) {
        let result = null;
    ...
    if (Number(editingRecipeIdInput.value || 0) === recipeId) resetRecipeForm();
    await loadRecipes();
}

async function batchImport() {
    const raw = batchJsonInput.value.trim();
    if (!raw) { batchStatus.textContent = '请粘贴 JSON 数据'; return; }
    let recipes;
    try { recipes = JSON.parse(raw); } catch { batchStatus.textContent = 'JSON 格式错误'; return; }
    if (!Array.isArray(recipes)) recipes = [recipes];
    batchStatus.textContent = `导入中（${recipes.length} 条）…`;
    try {
        const response = await fetch('/api/v1/recipes/batch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
            body: JSON.stringify({ recipes }),
        });
        const result = await response.json();
        if (!response.ok) throw new Error(getErrorMessage(result.detail, '导入失败'));
        batchStatus.textContent = `成功导入 ${result.length} 条菜谱`;
...
document.querySelectorAll('[data-add-row]').forEach(button => {
    button.addEventListener('click', () => {
        ...
        const target = key === 'ingredients' ? ingredientsList : keywordsList;
        const placeholder = key === 'ingredients' ? '例如：盐 2g' : '例如：清淡';
        addDynamicRow(target, placeholder);
    });
});

imageUploadButton?.addEventListener('click', async () => {
    try { await uploadRecipeImage(); } catch (error) { setFormStatus(error.message, 'error'); }
});
imageFileInput?.addEventListener('change', () => {
    const file = imageFileInput.files?.[0];
    if (!file) { setImagePreview(imageUrlInput.value); return; }
    setImagePreviewFromFile(file);
});
imageUrlInput?.addEventListener('input', () => {
    if (!imageFileInput.files?.length) setImagePreview(imageUrlInput.value);
});

document.getElementById('reload-list')?.addEventListener('click', async () => {
    try { await loadRecipes(); } catch (error) { recipeList.innerHTML = `<div class="empty-state md:col-span-2">${error.message}</div>`; recipeCount.textContent = '加载失败'; }
});

cancelEditButton?.addEventListener('click', () => { resetRecipeForm(); setFormStatus('已退出编辑模式', 'idle'); });
form.addEventListener('submit', submitRecipe);
batchImportBtn?.addEventListener('click', batchImport);
...
recipeList.addEventListener('click', async (event) => {
    ...
    if (editButton) {
        ...
        if (recipe) { fillForm(recipe); setFormStatus(`正在编辑：${recipe.name}`, 'busy'); }
        return;
    }
    ...
    if (!recipeId) return;
    if (!window.confirm(`确认删除食谱 #${recipeId} 吗？`)) return;
    try { await deleteRecipe(recipeId); } catch (error) { setFormStatus(error.message, 'error'); }
});
